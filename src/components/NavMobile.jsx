...
const NavMobile = () => {
  ...
  const circleVariants = {
    hidden: {scale:0},
    visible: {scale: 250, transition:{type: 'spring', stiffness: 160, damping: 60,},},
  }
        ...
        </div>
        {
          navigation.map((item, index) => {
            return <li key={index} className='mb-8'>
              <Link to={item.href} smooth={true} duration={500} offset={-70} className='text-xl cursor-pointer capitalize' onClick={() => setIsOpen(false)}>{item.name}</Link>
            </li>
          })
        }
      </motion.ul>
